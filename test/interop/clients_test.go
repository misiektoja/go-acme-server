...
func TestAcmezHTTP01(t *testing.T) {
	...
	h := newHarness(t, port, false)
	client, account := h.acmez(t, s)
	key := newKey(t)
	ctx, cancel := context.WithTimeout(t.Context(), 30*time.Second)
	defer cancel()
	certs, err := client.ObtainCertificateForSANs(ctx, account, key, []string{"issuance.test"})
	if err != nil || len(certs) != 1 {
		t.Fatalf("issuance = %v, %v", len(certs), err)
	}
	h.verify(t, certs[0].ChainPEM, &key.PublicKey)
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func TestAcmezRejectsWrongProof(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(t.Context(), 30*time.Second)
	defer cancel()
	if _, err := client.ObtainCertificateForSANs(ctx, account, newKey(t), []string{"issuance.test"}); err == nil {
	...
	for _, id := range ids {
		...
		if err != nil || order.Status != acmeserver.OrderInvalid || order.CertificateID != "" {
	...
	t.Log("incorrect HTTP proof rejected, orders invalid, no CA issuance")
}
...
func TestCertbotHTTP01(t *testing.T) {
	...
	h := newHarness(t, availablePort(t), false)
	key := newKey(t)
	csr, err := x509.CreateCertificateRequest(rand.Reader, &x509.CertificateRequest{DNSNames: []string{"issuance.test"}}, key)
	if err != nil {
		t.Fatal(err)
	}
	...
	command := exec.CommandContext(ctx, executable, "certonly", "--standalone", "--preferred-challenges", "http",
		"--http-01-address", "127.0.0.1", "--http-01-port", strconv.Itoa(h.port),
	...
	order := h.verify(t, readFile(t, fullchain), &key.PublicKey)
